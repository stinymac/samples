...
public class SpliteratorInAction {
    ...
    public static void main(String[] args) {
        ...
        SimpleTextSpliterator simpleTextSpliterator = new SimpleTextSpliterator(TEXT);
        Optional.ofNullable(simpleTextSpliterator.stream().count()).ifPresent(System.out::println);
        //simpleTextSpliterator.stream().forEach(System.out::println);
        simpleTextSpliterator.parallelStream().filter(s -> !"".equals(s)).forEach(System.out::println);
    }

    static class SimpleTextSpliterator {

        private final String[] data;

        public SimpleTextSpliterator(String text) {
        ...
        private class TextSpliterator implements Spliterator{

            private int start,end;

            public TextSpliterator() {
                this.start = 0;
                this.end = SimpleTextSpliterator.this.data.length - 1;
            }
            ...
            public boolean tryAdvance(Consumer action) {
                if (start <= end) {
                    action.accept(SimpleTextSpliterator.this.data[start++]);
                    return true;
                }
                return false;
            }
